...
class kNNAttention(nn.Module):
    ...
    def forward(self, x, islast):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn, self.softmax2(attention_scores)[:,:,:,0]
...
class Block(nn.Module):
    ...
    def forward(self, x, islast):
        atte_output, weight, contribution = self.attn(self.norm1(x), islast)
        x = x + self.drop_path(atte_output)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, weight, contribution
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.local_feature:
            ...
            ff_states, ff_weights, ff_contri = last_blk(concat, True)
            encoded = self.norm(ff_states)
